...
# lua-resty-digest-auth Native Linux Test Setup Script
# This script sets up a complete testing environment for the digest auth module on native Linux

set -e

echo "🚀 Setting up lua-resty-digest-auth test environment on native Linux..."
...
if [[ "$OS" == *"Ubuntu"* ]] || [[ "$OS" == *"Debian"* ]]; then
    ...
    sudo yum install -y wget curl gcc make pcre-devel openssl-devel zlib-devel
else
    print_warning "Unsupported distribution: $OS"
    print_status "Attempting to install common dependencies..."
    sudo apt install -y wget curl build-essential libpcre3-dev libssl-dev zlib1g-dev 2>/dev/null || \
    sudo yum install -y wget curl gcc make pcre-devel openssl-devel zlib-devel 2>/dev/null || \
    (print_error "Could not install dependencies automatically. Please install manually." && exit 1)
fi
...
if [[ "$OS" == *"Ubuntu"* ]] || [[ "$OS" == *"Debian"* ]]; then
    ...
    print_error "Unsupported distribution for OpenResty installation: $OS"
    print_status "Please install OpenResty manually from https://openresty.org/"
    exit 1
fi
...
cat > /tmp/digest-auth-test.service << 'EOF'
[Unit]
Description=Digest Auth Test Server
After=network.target

[Service]
Type=forking
User=root
ExecStart=/usr/local/openresty/bin/openresty -p /usr/local/openresty/nginx -c /usr/local/openresty/nginx/conf/nginx.conf
ExecReload=/usr/local/openresty/bin/openresty -p /usr/local/openresty/nginx -c /usr/local/openresty/nginx/conf/nginx.conf -s reload
ExecStop=/usr/local/openresty/bin/openresty -p /usr/local/openresty/nginx -c /usr/local/openresty/nginx/conf/nginx.conf -s stop
Restart=on-failure
...
sudo cp /tmp/digest-auth-test.service /etc/systemd/system/
...
echo "  sudo systemctl start digest-auth-test"
echo "  sudo systemctl stop digest-auth-test"
echo "  sudo systemctl enable digest-auth-test"
echo ""
echo "📁 Files created:"
...
echo "  /etc/systemd/system/digest-auth-test.service - Systemd service"
echo ""
echo "🌐 Test URLs:"
